...
- [ ] Text PDFs handled via pypdf; scanned PDFs and images via Pixtral Large 2411 (OpenRouter, single multimodal model + `file-parser` plugin with `mistral-ocr` engine for PDF); Tesseract Spanish as fallback (CS-054 **done** — AC re-anchored to single-call architecture, shipped extractor + retry/backoff + image-only Tesseract auto-fallback covered by `tests/test_ingestion_pixtral.py` + `tests/test_openrouter_client.py`; CS-053 still needs `--- PAGE N ---` separators + 500-char vision retry + 30s wall-clock timeout; CS-055 still needs mean-confidence gate at 60.0 + BVA)
