...
- [ ] Text PDFs handled via pypdf; scanned PDFs and images via Pixtral Large 2411 (OpenRouter, single multimodal model + `file-parser` plugin with `mistral-ocr` engine for PDF); Tesseract Spanish as fallback (CS-054 + CS-055 **done** — single-call Pixtral AC re-anchored; Tesseract mean-confidence gate at 60.0 + BVA at 59.9/60.0/60.1; CS-053 still needs `--- PAGE N ---` separators + 500-char vision retry + 30s wall-clock timeout)
