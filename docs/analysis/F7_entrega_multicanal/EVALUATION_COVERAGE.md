...
| US-03 WhatsApp delivery | `WhatsappSender` + Zavu template | COVERED |
...
| BR-08 disclaimer locations | Email body, WhatsApp welcome, report | COVERED |
| BR-09 no marketing | Design + ops policy | COVERED |
| BR-10 Zavu approved templates only | `ZAVU_TEMPLATE_NAME_*` config | COVERED |
...
| Integration: Zavu | ZavuClient (shared with F1) | COVERED |
...
| NFR TLS 1.2+ for all external | SmtpClient + ZavuClient + KMS | COVERED |
...
4. **WhatsApp template approval** — Meta approval is a process taking days. Operationally critical: ensure approval before production launch.
...
| Q-F7-02 | WhatsApp template approval timing | Submit during build; reject launch otherwise |
...
| Q-F7-08 | WhatsApp resend conversationally | F1 webhook responds with instructions, not direct processing |
...
- WhatsApp user has WhatsApp Business (no preview link expansion) — link still works.
